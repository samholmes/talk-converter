...
mediaRoutes.get('/media/:type/:file', async (c) => {
  try {
    ...
    let fp = path.join(base, file);
    
    // Check if it's a directory-based structure
    if (!file.endsWith('.mp4')) {
      // Assume it's a directory, look for video.mp4 inside
      fp = path.join(base, file, 'video.mp4');
    }
...
// Handle nested youtube/stream paths like /media/youtube/Stream_Name/video.mp4
mediaRoutes.get('/media/youtube/:dir/video.mp4', async (c) => {
  try {
    await ensureDirs();
    ...
    const fp = path.join(youtubeDir, dir, 'video.mp4');
    
    console.log('Media request (youtube nested):', { dir, fp });
    ...
    console.error('Media handler error (youtube nested):', error);
    return c.text('Internal Server Error: ' + (error instanceof Error ? error.message : String(error)), 500);
  }
});
...
mediaRoutes.get('/media/talks/:dir/video.mp4', async (c) => {
  try {
    ...
    const fp = path.join(talksDir, dir, 'video.mp4');
    
    console.log('Media request (nested):', { dir, fp });
    ...
    console.error('Media handler error (nested):', error);
    return c.text('Internal Server Error: ' + (error instanceof Error ? error.message : String(error)), 500);
  }
});
